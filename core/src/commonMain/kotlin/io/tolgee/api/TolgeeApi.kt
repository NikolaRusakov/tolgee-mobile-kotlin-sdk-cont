...
internal data object TolgeeApi {
  ...
  suspend fun getTranslations(
    ...
    val language = currentLanguage?.let {
      createLocale(it).toTag("-").ifBlank { null }
    }
    ...
    val decoded = fresh?.decodeTranslation(config, language, path)
    ...
    val cached = storage?.get(path)?.decodeToString()?.decodeTranslation(config, language, path)
